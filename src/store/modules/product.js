...
const actions = {
    loadListProduct({ commit }) {
        var newArr = [];
        axios.get("https://freshshopadmin.tk/api/all-products").then((response) => {
            commit("setListProduct", response.data.data);
        }, (error) => { console.log(error) });
    },
    loadCurrentProduct({ commit }, unique_id) {
        if (Number.isInteger(Number(unique_id))) {
            axios.get("https://freshshopadmin.tk/api/products/" + unique_id)
                .then((response) => {
    ...
    loadListProductPaginate({ commit, rootState }) {
        axios.get("https://freshshopadmin.tk/api/products" + "?limit=" + rootState.listQuery.limit + "&&page=" + rootState.listQuery.page)
            .then((response) => {
    ...
    loadListProOfCatePaginate({ commit, rootState }, category_unique_id) {
        if (Number.isInteger(Number(category_unique_id))) {
            axios.get("https://freshshopadmin.tk/api/products-with-category/" + category_unique_id
