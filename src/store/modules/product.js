...
const actions = {
    ...
    loadListProductPaginate({ commit, rootState }) {
        axios.get("https://goshopp.tk/api/products" + "?limit=" + rootState.listQuery.limit + "&&page=" + rootState.listQuery.page)
            .then((response) => {
    ...
    loadListProOfCatePaginate({ commit, rootState }, category_unique_id) {
        if (Number.isInteger(Number(category_unique_id))) {
            axios.get("https://goshopp.tk/api/products-with-category/" + category_unique_id
