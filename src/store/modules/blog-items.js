...
const actions = {
    loadListBlogItems({ commit }) {
        axios.get("https://fruitadmin.tk/api/all-blog-items").then((response) => {
            commit("setListBlogItems", response.data.data);
        }, (error) => { console.log(error) });
    },
    loadCurrentBlog({ commit }, unique_id) {
        if (Number.isInteger(Number(unique_id))) {
            axios.get("https://fruitadmin.tk/api/blog-items/" + unique_id)
                .then((response) => {
    ...
    loadListBlogItemsPaginate({ commit, rootState }) {
        axios.get("https://fruitadmin.tk/api/blog-items" + "?limit=" + rootState.listQuery.limit + "&&page=" + rootState.listQuery.page)
            .then((response) => {
    ...
    loadListBlogOfCategoryPaginate({ commit, rootState }, category_unique_id) {
        if (Number.isInteger(Number(category_unique_id))) {
            axios.get("https://fruitadmin.tk/api/paginate-blog-category/" + category_unique_id
